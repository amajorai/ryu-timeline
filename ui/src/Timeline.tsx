...
import { AppIconStack } from "./AppIconStack.tsx";
import {
	focusContextTs,
	getFrameDataUrl,
	getJournal,
...
import type { JournalCard, JournalSnapshot, TimelineEvent } from "./types.ts";
...
const TRACK_META: Record<number, { label: string; tint: string; bar: string }> =
	{
		...
		3: {
			label: "Window",
			tint: "bg-info/15 text-status-info dark:text-status-info",
			bar: "bg-info",
		},
		4: {
			label: "Audio",
		...
		6: {
			label: "Clipboard",
			tint: "bg-warning/15 text-status-warning dark:text-status-warning",
			bar: "bg-warning",
		},
		7: {
			label: "Filesystem",
			tint: "bg-success/15 text-status-success dark:text-status-success",
			bar: "bg-success",
		},
		8: {
			label: "Git",
		...
		11: {
			label: "Calendar",
			tint: "bg-destructive/15 text-status-destructive dark:text-status-destructive",
			bar: "bg-destructive",
		},
		12: {
			label: "Journal",
...
function formatClock(tsMicros: number): string {
	return formatTime(tsMicros / 1000, {
		hour: "2-digit",
		minute: "2-digit",
		second: "2-digit",
	});
...
export function Timeline() {
	const [viewMode, setViewMode] = useState<"replay" | "timeline">("replay");
	...
	const nowMicros = useViewClock(1000, playing) * 1000;
	...
	const openMomentInReplay = useCallback(
		(ts: number) => {
			setViewMode("replay");
			seekToTs(ts);
		},
	...
	return (
		<div className="flex h-full flex-col overflow-hidden">
			{/* The current scrubber stays the default; the second mode is a
			    chronological History-style feed over the same captured data. */}
			<div className="flex min-h-12 shrink-0 flex-wrap items-center justify-between gap-2 border-b px-3 py-2">
				<div className="min-w-0">
					<div className="font-medium text-sm">Timeline</div>
					<div className="truncate text-muted-foreground text-xs">
						{viewMode === "replay"
							? "Replay captured activity"
							: "Browse activity chronologically"}
					</div>
				</div>
				<div className="flex items-center gap-2">
					...
					>
						...
						<ToggleGroupItem
							...
							value="timeline"
						>
							<HugeiconsIcon className="size-3.5" icon={TimelineListIcon} />
							Timeline
						</ToggleGroupItem>
					</ToggleGroup>
					<Select
				...
				<div className="relative flex-1 overflow-auto">
					{bodyState ?? (
						<TimelineView
							data={data ?? []}
							journal={journal}
							onOpenInReplay={openMomentInReplay}
						/>
					)}
				</div>
			) : (
					...
					{showScrubber ? (
						<>
							...
							<TransportBar
								...
								onZoomIn={() =>
									setZoom((z) => clamp(z * 2, MIN_ZOOM, MAX_ZOOM))
								}
								onZoomOut={() =>
									setZoom((z) => clamp(z / 2, MIN_ZOOM, MAX_ZOOM))
								}
...
/**
 * A human-readable, vertical view over the same range as the replay lanes.
 * Prefer the derived journal cards because they collapse noisy capture signals
 * into the meaningful blocks a person actually wants to scan. Raw events remain
 * available as a graceful fallback when the journal endpoint is unavailable.
 */
function TimelineView(props: {
	data: TimelineEvent[];
	journal: JournalSnapshot | null | undefined;
	onOpenInReplay: (ts: number) => void;
}) {
	const { data, journal, onOpenInReplay } = props;
	const journalGroups = useMemo(
		() => groupJournalCards(journal?.cards ?? []),
		[journal?.cards]
	);
	const eventGroups = useMemo(() => groupTimelineEvents(data), [data]);
	const hasJournal = journalGroups.length > 0;
	const totalItems = hasJournal ? (journal?.cards.length ?? 0) : data.length;

	return (
		<div className="min-h-full bg-muted/10" data-testid="timeline-history-view">
				...
				<div className="flex items-end justify-between gap-3">
					<div>
						...
						<p className="text-muted-foreground text-sm">
							{hasJournal
								? "Your recent work, grouped into focus blocks."
								: "Your recent captured activity, newest first."}
						</p>
					</div>
					<span className="shrink-0 rounded-full bg-muted px-2.5 py-1 font-mono text-muted-foreground text-xs tabular-nums">
						{totalItems} {hasJournal ? "blocks" : "events"}
					</span>
				</div>
					...
					? journalGroups.map((group) => (
							<section className="relative" key={group.key}>
								...
								<div className="relative space-y-3">
									<div
										aria-hidden="true"
										className="absolute top-5 bottom-5 left-16 w-px bg-border"
									/>
									{group.items.map((card) => (
										<JournalTimelineCard
											card={card}
											key={card.id}
											onOpen={onOpenInReplay}
										/>
									))}
								</div>
							</section>
							...
							<section className="relative" key={group.key}>
								...
								<div className="relative space-y-3">
									...
									{group.items.map((event, index) => (
										<EventTimelineCard
											event={event}
											key={`${event.ts}-${event.track}-${event.event_type}-${index}`}
											onOpen={onOpenInReplay}
										/>
									))}
								</div>
...
function TimelineDayHeading(props: { count: number; label: string }) {
	return (
...
const JournalTimelineCard = memo(function JournalTimelineCard(props: {
	card: JournalCard;
	onOpen: (ts: number) => void;
}) {
	const { card, onOpen } = props;
	const dotClass = card.distraction ? "bg-warning" : "bg-primary";
	return (
		<Button
			aria-label={`Open ${card.title} in replay`}
			className="group relative grid w-full grid-cols-[4rem_minmax(0,1fr)] gap-4 text-left"
			onClick={() => onOpen(card.start_ts)}
			type="button"
		>
			<time
				className="pt-4 text-right font-mono text-[11px] text-muted-foreground tabular-nums"
				dateTime={new Date(card.start_ts / 1000).toISOString()}
			>
				{formatClock(card.start_ts)}
			</time>
			<div className="relative rounded-xl border bg-card p-4 shadow-sm transition-colors group-hover:border-primary/40 group-hover:bg-accent/40">
				<span
					aria-hidden="true"
					className={`absolute top-5 -left-[1.32rem] size-2.5 rounded-full ring-4 ring-background ${dotClass}`}
				/>
				<div className="flex flex-wrap items-center gap-2">
					<span
						className={`rounded-full px-2 py-0.5 font-medium text-[10px] ${
							card.distraction
								? "bg-warning/15 text-status-warning"
								: "bg-primary/15 text-primary"
						}`}
					>
						{card.category}
					</span>
					<AppIconStack apps={card.apps ?? []} />
					<span className="text-muted-foreground text-xs">
						{card.primary_app}
					</span>
					<span className="ml-auto text-muted-foreground text-xs">
						{formatMinutes(card.end_ts - card.start_ts)} · {card.event_count}{" "}
						events
					</span>
				</div>
				<h4 className="mt-2 font-medium text-sm leading-snug">{card.title}</h4>
				<p className="mt-1 line-clamp-3 text-muted-foreground text-sm leading-5">
					{card.detailed_summary || card.summary}
				</p>
				<div className="mt-3 flex items-center justify-between gap-2 text-muted-foreground text-xs">
					<span>{formatClock(card.end_ts)} end</span>
...
const EventTimelineCard = memo(function EventTimelineCard(props: {
	event: TimelineEvent;
	onOpen: (ts: number) => void;
}) {
	const { event, onOpen } = props;
	const meta = trackMeta(event.track);
	return (
		<Button
			aria-label={`Open ${eventLabel(event)} in replay`}
			className="group relative grid w-full grid-cols-[4rem_minmax(0,1fr)] gap-4 text-left"
			onClick={() => onOpen(event.ts)}
			type="button"
		>
			<time
				className="pt-4 text-right font-mono text-[11px] text-muted-foreground tabular-nums"
			...
			<div className="relative rounded-xl border bg-card p-4 shadow-sm transition-colors group-hover:border-primary/40 group-hover:bg-accent/40">
				<span
					aria-hidden="true"
					className={`absolute top-5 -left-[1.32rem] size-2.5 rounded-full ring-4 ring-background ${meta.bar}`}
				/>
				<div className="flex flex-wrap items-center gap-2">
					<span
						className={`rounded-full px-2 py-0.5 font-medium text-[10px] ${meta.tint}`}
					>
						{meta.label}
					</span>
					<span className="text-muted-foreground text-xs">
						{event.app_name || "Captured activity"}
					</span>
				</div>
				<h4 className="mt-2 font-medium text-sm leading-snug">
					{eventLabel(event)}
				</h4>
				...
				<div className="mt-3 text-primary text-xs opacity-0 transition-opacity group-hover:opacity-100">
					Open in Replay →
				</div>
			</div>
		</Button>
		...
		<div className="grid shrink-0 gap-3 border-b bg-background px-4 py-3 lg:grid-cols-[minmax(0,1.2fr)_minmax(340px,0.8fr)]">
			<div className="min-w-0">
				<div className="mb-2 flex items-center justify-between gap-2">
					<div className="font-medium text-sm">Work journal</div>
					<div className="flex items-center gap-2">
						<span className="text-muted-foreground text-xs">
...
function StatList(props: { stats: JournalSnapshot["apps"]; title: string }) {
	...
	return (
		<div className="min-w-0 rounded-lg bg-muted/30 p-3">
			<div className="mb-1.5 font-medium text-xs">{title}</div>
